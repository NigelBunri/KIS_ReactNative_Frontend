...
const resolveConfiguredEngineFlowKeys = (
  engines: BookingEngineDescriptor[],
): string[] =>
  Array.from(
    new Set(
      engines
...
const resolveServiceEngineDescriptors = (
  service: ServiceDefinition,
...
const resolveServiceConfiguredFlowKeys = (
  service: ServiceDefinition,
): string[] => {
  const tokens = [
    ...normalizeStringList(service?.availableEngines),
...
const normalizeService = (
  ...
  const cents = Number.isFinite(Number(centsRaw))
    ? Number(centsRaw)
    : undefined;
  const mediumNames = Array.from(
    new Set([
      ...normalizeStringList(raw?.mediumNames),
      ...normalizeStringList(raw?.medium_names),
      ...extractServiceEngineTokensFromRaw({
        medium_links: raw?.medium_links,
        mediumLinks: raw?.mediumLinks,
      }),
    ]),
  );
...
const resolveInstitutionServices = (institution: any): ServiceDefinition[] => {
  ...
  const normalizedType = (() => {
    const type = String(institution?.type || '')
      .trim()
      .toLowerCase();
    ...
    if (
      HEALTH_DASHBOARD_INSTITUTION_TYPES.includes(
        type as HealthDashboardInstitutionType,
      )
    ) {
  ...
  return sanitizeServiceList([
    ...(HEALTH_DASHBOARD_DEFAULT_SERVICES[normalizedType] || []),
  ]);
};
...
const getCardTimeOptions = (
  card: Pick<HealthCardRow, 'accessMode' | 'timeOptions' | 'timeValue'>,
): string[] => {
...
const getCardSortTimestamp = (
  card: Pick<
    HealthCardRow,
    'dateKey' | 'accessMode' | 'timeOptions' | 'timeValue'
  >,
) => {
...
const buildCards = (
  ...
  const services = resolveInstitutionServices(institution).filter(
  ...
  Object.entries(dateServiceIds).forEach(([dateKey, ids]) => {
    ...
    const timeOptions =
      accessMode === 'slots'
        ...
        : [];
    const timeValue =
      accessMode === 'all_day' ? 'All day' : timeOptions.join(', ');
  ...
  rows.sort((a, b) => {
    ...
    return String(a.service.name || '').localeCompare(
...
const enrichCardsWithAvailability = (
  ...
  return cards.map(card => {
    ...
    const accessMode: 'slots' | 'all_day' =
      modeFromAvailability === 'all_day' ? 'all_day' : card.accessMode;
    const options =
      accessMode === 'slots'
        ? normalizeTimeList(
...
const toCardScheduleKey = (card: Pick<HealthCardRow, 'dateKey' | 'service'>) =>
  `${String(card?.dateKey || '').trim()}:${String(
    card?.service?.id || '',
  ).trim()}`;
...
const mergeCardRows = (
  ...
  const upsert = (
    ...
    const accessMode: 'slots' | 'all_day' =
      row.accessMode === 'all_day' ? 'all_day' : 'slots';
    const timeOptions = accessMode === 'slots' ? getCardTimeOptions(row) : [];
    const normalizedService = sanitizeServiceEngineFields({
      id: serviceId,
      name:
        String(row?.service?.name || '').trim() ||
        prettifyServiceId(serviceId) ||
        'Health Service',
    ...
    const mergedAccessMode: 'slots' | 'all_day' =
      existing.accessMode === 'all_day' ||
      normalizedRow.accessMode === 'all_day'
        ? 'all_day'
        : 'slots';
    const mergedTimes =
    ...
    const shouldUseSecondaryService =
      ...
      (String(existing.service?.name || '').trim() === '' ||
        ...
        String(existing.service?.name || '').trim() ===
          prettifyServiceId(existing.service.id));
    ...
    const mergedService = sanitizeServiceEngineFields({
    ...
    map.set(key, {
      ...existing,
      ...(source === 'secondary' && normalizedRow.id
        ? { id: normalizedRow.id }
        : {}),
      ...
      timeValue:
        mergedAccessMode === 'all_day' ? 'All day' : mergedTimes.join(', '),
    });
  };

  primary.forEach((card, index) => upsert(card, 'primary', index));
  secondary.forEach((card, index) =>
    upsert(card, 'secondary', index + primary.length),
  );
...
const normalizeRatings = (raw: any): ServiceRatingEntry[] => {
  ...
  raw.forEach((item, index) => {
    ...
    rows.push({
      ...
      userId,
      userName:
        String(item.userName ?? item.user_name ?? 'User').trim() || 'User',
...
const resolveMembershipSettings = (institution: any) => {
  const settings =
    institution?.membership_settings ?? institution?.membershipSettings ?? {};
  const openValue =
    settings?.open ??
    settings?.is_open ??
...
export default function HealthInstitutionCardsScreen({
  navigation,
  route,
}: Props) {
  ...
  const [membershipDiscountPercent, setMembershipDiscountPercent] =
    useState(10);
  ...
  const [availabilityOverride, setAvailabilityOverride] = useState<Record<
    string,
    unknown
  > | null>(null);
  ...
  const [timePickerCard, setTimePickerCard] = useState<HealthCardRow | null>(
    null,
  );
  ...
  const [configuredServiceEngines, setConfiguredServiceEngines] = useState<
    Record<string, BookingEngineDescriptor[]>
  >({});
  const serviceEngineCacheRef = useRef<
    Record<string, BookingEngineDescriptor[]>
  >({});
  ...
  const institution =
    institutionIndex >= 0 ? institutions[institutionIndex] : null;
  ...
  const localLandingDraft = useMemo(
    () => resolveLandingDraft(institution),
    [institution],
  );
  ...
  const landingPublished = landingPageResolved
    ? landingPagePublished
    : inferredLandingPublished;
  ...
  const cards = useMemo(() => {
    ...
    const backendForMerge =
      localCards.length > 0
        ? backendCards.filter(card =>
            localScheduleKeys.has(toCardScheduleKey(card)),
          )
        : backendCards;
  ...
  const fetchCardsFromBackend = useCallback(
    async (availabilitySnapshot?: Record<string, unknown> | null) => {
      ...
      const rawCards: HealthCardRow[] = apiCards.map(
        (item: any, index: number) => {
          const service = normalizeService(item?.service, index) ||
            sanitizeServiceEngineFields({
              ...
              active: true,
              basePriceCents: Number.isFinite(
          ...
          const cardId = String(
            item?.id || `${dateKey}:${service.id}:${index}`,
          );
          const explicitBroadcasted = !!(
            item?.isBroadcasted ||
            item?.is_broadcasted ||
            item?.broadcasted
          );
          const inferredBroadcasted = isCardIdBroadcasted(
            cardId,
            apiBroadcastedCardIds,
          );
          const accessModeRaw = item?.access_mode ?? item?.accessMode;
          const rawTimeValue = item?.time ?? item?.timeValue;
          const accessMode: 'slots' | 'all_day' =
            isAllDayValue(accessModeRaw) ||
            isAllDayValue(rawTimeValue) ||
            !!item?.all_day ||
            !!item?.allDay
              ? 'all_day'
              : 'slots';
      ...
      rawCards.forEach((row, index) => {
        ...
        if (!existing) {
          dedupedMap.set(dedupeKey, {
            ...
            timeOptions:
              row.accessMode === 'all_day'
                ? []
                : normalizeTimeList(row.timeOptions),
            timeValue:
              row.accessMode === 'all_day'
                ? 'All day'
                : normalizeTimeList(row.timeOptions).join(', '),
          });
          return;
        }
  ...
  const loadState = useCallback(
    async (options?: { showLoader?: boolean }) => {
      ...
      try {
        ...
        const list = Array.isArray(health.profile?.institutions)
          ? health.profile.institutions
          : [];
        setInstitutions(list);
        const currentInstitution = list.find(
          (entry: any) => String(entry?.id) === String(institutionId),
        );
        ...
        setRatings(
          normalizeRatings(
            currentInstitution?.service_ratings ??
              currentInstitution?.serviceRatings,
          ),
        );
        ...
        try {
          ...
          const availabilityRes = await fetchInstitutionAvailability(
            institutionId,
          );
          ...
          if (
            ...
            availabilitySnapshot = availabilityPayload as Record<
              string,
              unknown
            >;
        ...
        Alert.alert(
          'Health cards',
          error?.message || 'Unable to load health cards.',
        );
      } finally {
  ...
  const saveMembershipSettings = useCallback(async () => {
    if (!canManageMembership) {
      Alert.alert(
        'Health cards',
        'Your role cannot change membership settings.',
      );
      return;
    }
    ...
    try {
      ...
      if (!response?.success) {
        throw new Error(
          response?.message || 'Unable to save membership settings.',
        );
      }
      ...
      Alert.alert(
        'Health cards',
        error?.message || 'Unable to save membership settings.',
      );
    } finally {
      setSaving(false);
    }
  ...
  const handleJoinInstitution = useCallback(async () => {
    if (!membershipOpen) {
      Alert.alert(
        'Membership',
        'Membership is currently closed for this institution.',
      );
      return;
    }
    if (!currentUserId) {
      Alert.alert(
        'Membership',
        'You must be signed in to join this institution.',
      );
      return;
    }
  ...
  const startBookingForCard = useCallback(
    async (card: HealthCardRow, selectedTime?: string) => {
      ...
      const inferredEnginesForFallback = resolveServiceEngineDescriptors(
        card.service,
      );
      ...
      const memberPriceCents = Number.isFinite(
        Number(card.service.basePriceCents),
      )
        ? Math.round(
            (Number(card.service.basePriceCents) *
              (100 - membershipDiscountPercent)) /
              100,
          )
        : undefined;
  ...
  const toggleBroadcastCard = useCallback(
    async (card: HealthCardRow) => {
      ...
      setBackendCards(prev =>
        prev.map(row =>
          row.id === card.id ? { ...row, isBroadcasted: true } : row,
        ),
      );
      try {
        const response = await postRequest(
        ...
        if (!response?.success) {
          throw new Error(
            response?.message || 'Unable to update card broadcast status.',
          );
        }
        await fetchCardsFromBackend();
      } catch (error: any) {
        setBackendCards(prev =>
          prev.map(row =>
            row.id === card.id ? { ...row, isBroadcasted: false } : row,
          ),
        );
        Alert.alert(
          'Broadcast',
          error?.message || 'Unable to update card broadcast status.',
        );
      } finally {
        setSaving(false);
      }
        ...
        >
          ...
          {canManageMembership ? (
            ...
            >
              ...
              >
                Open or close institution membership and set member discount
                (10% to 100%).
              </Text>
              <View
                style={{
              ...
              >
                ...
                <TouchableOpacity
                  onPress={() =>
                    setMembershipDiscountPercent(prev =>
                      clampDiscount(prev - 5),
                    )
                  }
                  disabled={!canManageMembership || saving}
                  style={{
              ...
              >
                Your role: {actorRole} · Current membership:{' '}
          ...
          >
            ...
            >
              {(['today', 'upcoming', 'past'] as CardFilterKey[]).map(
                filterKey => (
                  <KISButton
                    key={filterKey}
          ...
          <View style={{ marginTop: spacing.md, gap: spacing.sm }}>
            {filteredCards.map(card => {
              ...
              const serviceRatings =
                ratingsByService.get(String(card.service.id)) || [];
              const average =
                serviceRatings.length > 0
                  ? serviceRatings.reduce(
                  ...
                  <View style={{ padding: spacing.md }}>
                    ...
                    <View style={{ marginTop: spacing.xs }}>
                      ...
                      {isCurrentUserMember ? (
                        ...
                        >
                          Member price ({membershipDiscountPercent}% off):{' '}
                    ...
                    >
                      ...
                      >
                        Engines auto-run based on the workflow when you tap Book
                        Now.
                      </Text>
                    </View>
                        ...
                        {[1, 2, 3, 4, 5].map(star => {
                          ...
                          return (
                            <TouchableOpacity
                              ...
                              onPress={() => {
                                submitServiceRating(card.service, star).catch(
                                  () => undefined,
                                );
                              }}
          ...
          {filteredCards.length === 0 ? (
            ...
            >
              ...
              >
                Health cards are produced from scheduled availability dates with
                selected services.
              </Text>
              <View style={{ marginTop: spacing.sm }}>
                <KISButton
          ...
          >
            ...
            >
              ...
              <Text style={{ ...typography.caption, color: palette.subtext }}>
                {timePickerCard.service.name} •{' '}
                {toDateLabel(timePickerCard.dateKey)}
              </Text>
              <View
                style={{
                  flexDirection: 'row',
