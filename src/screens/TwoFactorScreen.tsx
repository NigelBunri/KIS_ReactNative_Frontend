...
export default function TwoFactorScreen({ onBack }: Props) {
  ...
  const startSetup = useCallback(async () => {
    ...
    if (res?.success) {
      ...
      Alert.alert('Error', res?.message || 'Setup failed');
    }
  }, []);
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            setSaving(true);
            const res = await postRequest(ROUTES.auth.twoFactorDisable, {}, { errorMessage: 'Disable failed' });
            setSaving(false);
      ...
      <View style={styles.content}>
        <View style={[styles.statusCard, { backgroundColor: palette.card, borderColor: palette.inputBorder }]}>
          <View style={styles.statusRow}>
            <View style={[styles.dot, { backgroundColor: status?.enabled ? '#22c55e' : '#f87171' }]} />
          ...
          <View style={{ gap: 12 }}>
            ...
            >
              {saving ? (
                ...
                <Text style={[styles.primaryBtnText, { color: code.length === 6 ? '#fff' : palette.subtext }]}>
                  Verify & Enable
                </Text>
              )}
            </Pressable>
        ...
        {status?.enabled && step === 'idle' && (
          <Pressable
            ...
            disabled={saving}
            style={[styles.dangerBtn, { borderColor: palette.danger ?? '#d9534f' }]}
          >
            {saving ? (
              <ActivityIndicator size="small" color={palette.danger ?? '#d9534f'} />
            ) : (
              <Text style={[styles.dangerBtnText, { color: palette.danger ?? '#d9534f' }]}>
