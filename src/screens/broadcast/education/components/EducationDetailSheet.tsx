...
type AssessmentDraft = Record<
  string,
  { answer_text?: string; selected_option_ids: string[] }
>;
...
const normalizeSummaryItems = (items: any[] | undefined) =>
...
const InsightsStrip = ({
  ...
  const cards = [
    ...
    {
      label: 'Average score',
      value: `${Math.round(insights.averageScorePercent || 0)}%`,
    },
    {
      label: 'Certificate',
      value: insights.certificateReady
        ? 'Ready'
        : `${insights.certificateProgressPercent}%`,
    },
  ];
  return (
    <View style={{ marginTop: 16 }}>
      <Text
        style={{ color: palette.text, fontWeight: '800', marginBottom: 10 }}
      >
        Learner insights
      </Text>
...
const DetailSummaryCard = ({
  ...
  palette: any;
  summary?: EducationDetailSummary | null;
}) => {
  if (!summary) return null;
  const highlights = normalizeSummaryItems(summary.highlights);
    ...
    >
      ...
      {highlights.length ? (
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 10 }}>
          {highlights.map(summaryItem => (
            <View
              key={`${summaryItem.label}-${summaryItem.value}`}
              style={{
                minWidth: 130,
                flexGrow: 1,
                borderRadius: 18,
...
export default function EducationDetailSheet({
  visible,
  item,
  onClose,
  onEnroll,
  ...
  const [certificateLocalUri, setCertificateLocalUri] = useState<string | null>(
    null,
  );
  const [certificateLoading, setCertificateLoading] = useState(false);
  const [workspaceSection, setWorkspaceSection] = useState<
    'overview' | 'path' | 'current' | 'certificate'
  >('overview');
  ...
  const [insightsState, setInsightsState] =
    useState<EducationLearnerInsights | null>(content?.insights || null);
  const [activeModuleId, setActiveModuleId] = useState('');
  const [selectedItemId, setSelectedItemId] = useState('');
  const [progressBusy, setProgressBusy] = useState(false);
  const [assessmentSubmission, setAssessmentSubmission] = useState<any | null>(
    null,
  );
  const [assessmentDraft, setAssessmentDraft] = useState<AssessmentDraft>({});
  const [previewMaterial, setPreviewMaterial] = useState<any | null>(null);
  const [viewerEnrollment, setViewerEnrollment] = useState<any | null>(
    (content as any)?.viewerState?.enrollment ?? null,
  );
  const [viewerBooking, setViewerBooking] = useState<any | null>(
    (content as any)?.viewerState?.booking ?? null,
  );
  ...
  useEffect(() => {
    ...
    const nextModuleId =
      nextProgress?.currentModuleId ||
      content?.currentModule?.id ||
      outline[0]?.id ||
      '';
    setActiveModuleId(nextModuleId);
    const nextItemId =
      nextProgress?.currentItemId ||
  ...
  const certificateReady = Boolean(
    content?.certificate?.ready || insightsState?.certificateReady,
  );
  const certificateIssuedAt = content?.certificate?.issuedAt || null;
  const certificateUrl = content?.id
    ? ROUTES.education.certificate(content.id)
    : null;
  const detailSummary =
  ...
  const sectionChips = [
    ...
    { key: 'path', label: 'Path' },
    ...(enrolled ? [{ key: 'current', label: 'Continue' }] : []),
  ...
  const handleShareCertificate = async () => {
    ...
    try {
      const response = await getRequest(
        `${ROUTES.education.certificate(content.id)}?format=json`,
        {
      ...
      const shareUrl =
        payload?.share_url ||
        (payload?.certificate_share_token
          ? ROUTES.education.certificateShare(payload.certificate_share_token)
          : null);
      await Share.share({
        ...
        message: [
          ...
          certificateIssuedAt
            ? `Issued: ${formatDateTime(certificateIssuedAt)}`
            : null,
      ...
      Alert.alert(
        'Certificate',
        error?.message || 'Unable to share certificate right now.',
      );
    }
  };
  ...
  const applyLearningPayload = async (payload: any) => {
    ...
    if (payload?.current_module?.id)
      setActiveModuleId(payload.current_module.id);
    else if (progress?.currentModuleId)
      setActiveModuleId(progress.currentModuleId);
  ...
  const updateProgress = async (
    ...
    if (!response?.success) {
      Alert.alert(
        'Education',
        response?.message || 'Unable to update learning progress.',
      );
      return;
    }
  ...
  const openResource = async (url?: string) => {
    if (!hasLearningAccess) {
      Alert.alert(
        'Education',
        'Enroll first to open or download learning resources.',
      );
      return;
    }
    if (!url) {
      Alert.alert(
        'Education',
        'No resource link is available for this item yet.',
      );
      return;
    }
  ...
  const downloadResource = async (
    ...
    if (!hasLearningAccess) {
      Alert.alert('Education', 'Enroll first to download learning resources.');
      return;
    }
    if (!url) {
      Alert.alert('Education', 'No downloadable file is available yet.');
      return;
    }
    try {
      setResourceBusy(true);
      const downloadsDir = `${RNFS.DocumentDirectoryPath}/kis-education-downloads`;
      await RNFS.mkdir(downloadsDir);
      const urlTail = url.split('/').pop()?.split('?')[0] || 'resource';
      const cleanHint = (fileNameHint || urlTail || 'resource')
        .replace(/[^a-zA-Z0-9._-]/g, '_')
        .replace(/_+/g, '_');
      const filePath = `${downloadsDir}/${cleanHint}`;
      ...
      Alert.alert('Download complete', cleanHint, [
        { text: 'Close', style: 'cancel' },
        {
          text: 'Open',
          onPress: () => {
            Linking.openURL(localUri).catch(() => {
              Share.share({ url: localUri, message: localUri }).catch(() => {});
            });
          },
        },
      ]);
  ...
  const handleMarkSatisfied = async () => {
    ...
    try {
      ...
      const nextBooking = response?.data?.booking ?? response?.booking ?? null;
      if (nextBooking) {
        setViewerBooking(nextBooking);
      }
      Alert.alert('Education', 'Payment released to the provider.');
      await onRefreshProgress?.();
    } catch (error: any) {
      Alert.alert(
        'Education',
        error?.message || 'Unable to confirm satisfaction right now.',
      );
    } finally {
  ...
  const handleLearnerItemAction = async (
    action: string,
    ...
    if (!response?.success) {
      Alert.alert(
        'Education',
        response?.message || 'Unable to complete this learning action.',
      );
      return;
    }
    await applyLearningPayload(response?.data || response || {});
  ...
  const buildAssessmentResponsesPayload = () => {
    ...
    const questions = Array.isArray((selectedItem.content as any)?.questions)
      ? (selectedItem.content as any).questions
      : [];
    return questions.map((question: any) => ({
      question_id: question.id,
      answer_text: assessmentDraft[question.id]?.answer_text || '',
      selected_option_ids:
        assessmentDraft[question.id]?.selected_option_ids || [],
    }));
  };
    ...
    return (
      ...
      >
        ...
        <Text style={{ color: palette.subtext, marginTop: 8 }}>
          ...
          {materialPayload?.resource_name
            ? ` · ${materialPayload.resource_name}`
            : ''}
        </Text>
        {mime ? (
          <Text style={{ color: palette.subtext, marginTop: 4 }}>{mime}</Text>
        ) : null}
        {options?.summary ? (
          <Text
            style={{ color: palette.subtext, marginTop: 10, lineHeight: 20 }}
          >
            {options.summary}
          </Text>
        ) : null}
        {!hasLearningAccess ? (
          ...
          >
            <Text style={{ color: palette.subtext, lineHeight: 19 }}>
              Enroll first to view or download this material.
            </Text>
          </View>
        ) : null}
        {resourceUrl && kind === 'video' && hasLearningAccess ? (
        ...
        {resourceUrl && kind === 'audio' && hasLearningAccess ? (
          <View style={{ marginTop: 14 }}>
            <Video
              source={mediaSource}
              style={{
        ...
        {resourceUrl && kind === 'pdf' && hasLearningAccess ? (
          <View
            style={{
              marginTop: 14,
              height: 420,
        ...
        {resourceUrl &&
        hasLearningAccess &&
        !['video', 'audio', 'pdf', 'image'].includes(kind) ? (
          <View
            style={{
              marginTop: 14,
              borderRadius: 16,
          ...
          >
            <Text style={{ color: palette.subtext, lineHeight: 19 }}>
              This file type does not have an inline viewer. Use the button
              below to open it externally.
            </Text>
          </View>
        ) : null}
        <View
        ...
        >
          <KISButton
            ...
            size="sm"
            variant="outline"
            disabled={!resourceUrl || !hasLearningAccess}
          ...
          <KISButton
            ...
            onPress={() =>
              void downloadResource(
                resourceUrl,
                materialPayload?.resource_name || options?.title,
              )
            }
          />
        </View>
  ...
  const renderConsumptionPanel = () => {
    ...
    if (learningItem.type === 'lesson') {
      return (
        <View style={sectionStyle}>
          ...
          <Text
            style={{ color: palette.subtext, marginTop: 8, lineHeight: 22 }}
          >
            {contentPayload.content ||
              learningItem.summary ||
              'This lesson does not have body content yet.'}
          </Text>
          {Array.isArray(contentPayload.materials) &&
          contentPayload.materials.length ? (
            <View style={{ marginTop: 14 }}>
              <Text
                style={{
                  color: palette.text,
    ...
    if (learningItem.type === 'class_session') {
      return (
        <View style={sectionStyle}>
          ...
          <Text style={{ color: palette.subtext, marginTop: 8 }}>
            Starts: {formatDateTime(contentPayload.starts_at)}
          </Text>
          <Text style={{ color: palette.subtext, marginTop: 4 }}>
            Ends: {formatDateTime(contentPayload.ends_at)}
          </Text>
          <Text style={{ color: palette.subtext, marginTop: 4 }}>
            Mode: {formatTypeLabel(contentPayload.delivery_mode || 'scheduled')}
          </Text>
          {contentPayload.location_text ? (
            <Text style={{ color: palette.subtext, marginTop: 4 }}>
              Location: {contentPayload.location_text}
            </Text>
          ) : null}
          <View
            style={{
    ...
    if (learningItem.type === 'assessment') {
      const questions = Array.isArray(contentPayload.questions)
      ...
      return (
        <View style={sectionStyle}>
          ...
          {contentPayload.instructions ? (
            <Text
              style={{ color: palette.subtext, marginTop: 8, lineHeight: 20 }}
            >
              {contentPayload.instructions}
            </Text>
          ) : null}
          <Text style={{ color: palette.subtext, marginTop: 8 }}>
            {formatTypeLabel(contentPayload.assessment_type || 'assessment')} ·{' '}
            {contentPayload.duration_minutes ?? 0} mins ·{' '}
            {contentPayload.question_count ?? 0} questions
          </Text>
          {assessmentSubmission ? (
            ...
            >
              Attempt #{assessmentSubmission.attempt_number} ·{' '}
              {formatTypeLabel(assessmentSubmission.status || 'started')}
              {assessmentSubmission.score_percent !== null &&
              assessmentSubmission.score_percent !== undefined
          ...
          assessmentSubmission.responses.some(
            (response: any) =>
              response?.grader_feedback || response?.earned_points !== null,
          ) ? (
          ...
          <View style={{ marginTop: 14 }}>
            {questions.slice(0, 8).map((question: any, index: number) => {
              const answer = assessmentDraft[question.id] || {
                answer_text: '',
                selected_option_ids: [],
              };
              const hasOptions =
                  ...
                  {hasOptions ? (
                    <View style={{ marginTop: 10 }}>
                      {question.options.map((option: any) => {
                        const selected = answer.selected_option_ids?.includes(
                          option.id,
                        );
                        return (
                          <Pressable
                      ...
                      onChangeText={value =>
                        setAssessmentDraft(prev => ({
                          ...
                          [question.id]: {
                            answer_text: value,
                            selected_option_ids:
                              prev[question.id]?.selected_option_ids || [],
                          },
                        }))
                      }
                      multiline
          ...
          <View style={{ flexDirection: 'row', gap: 8, flexWrap: 'wrap' }}>
            {!assessmentSubmission ? (
              <KISButton
                title="Start attempt"
                size="sm"
                onPress={() => void handleLearnerItemAction('start_assessment')}
                disabled={progressBusy}
              />
            ) : null}
            <KISButton
              ...
              variant="outline"
              onPress={() =>
                void handleLearnerItemAction('save_assessment', {
                  responses: buildAssessmentResponsesPayload(),
                })
              }
            ...
            <KISButton
              ...
              size="sm"
              onPress={() =>
                void handleLearnerItemAction('submit_assessment', {
                  responses: buildAssessmentResponsesPayload(),
                })
              }
              disabled={
                progressBusy ||
                !assessmentSubmission ||
                assessmentSubmission?.status !== 'started'
              }
            />
          </View>
        </View>
    ...
    if (content.type === 'workshop') {
      ...
      const eventType = toText(
        (content as any)?.eventType || (content as any)?.broadcastKind,
      );
      ...
      return (
        <View style={sectionStyle}>
          ...
          <Text style={{ color: palette.subtext, marginTop: 8 }}>
            Starts: {formatDateTime(scheduleStartsAt)}
          </Text>
          <Text style={{ color: palette.subtext, marginTop: 4 }}>
            Ends: {formatDateTime(scheduleEndsAt)}
          </Text>
          {timezoneName ? (
            <Text style={{ color: palette.subtext, marginTop: 4 }}>
          ...
          <Text style={{ color: palette.subtext, marginTop: 4 }}>
            Capacity: {formatSeatLimit(seatLimit)}
          </Text>
          <View
            style={{
              flexDirection: 'row',
        ...
        >
          ...
          >
              ...
              {(content as any)?.detailLoading ? (
                ...
                >
                  ...
                  <Text style={{ color: palette.subtext }}>
                    Loading full learner workspace…
                  </Text>
                </View>
              ) : null}
              <View style={{ marginTop: 16 }}>
                ...
                >
                  <View
                    style={{
                      width: `${Math.max(
                        3,
                        Math.min(100, progressPercent || 3),
                      )}%`,
                      height: '100%',
                      borderRadius: 999,
                ...
                >
                  ...
                  >
                    ...
                    >
                      {outline.length} module{outline.length === 1 ? '' : 's'}
                    </Text>
                  </View>
                  <View
                    style={{
                  ...
                  >
                    ...
                    >
                      {flatItems.length} learning item
                  ...
                  >
                    ...
                    >
                      <KISButton
                        ...
                        size="sm"
                        onPress={() => setCertificateVisible(value => !value)}
                      />
                      <KISButton
                        title="Open PDF"
                        size="sm"
                        variant="outline"
                        disabled={!certificateLocalUri}
                        onPress={() =>
                          certificateLocalUri &&
                          Linking.openURL(certificateLocalUri)
                        }
                      />
                    </View>
                  </View>
                  ...
                  >
                    ...
                    {viewerEnrollment?.status ? (
                      <Text style={{ color: palette.subtext, marginTop: 6 }}>
                        Enrollment:{' '}
                        {String(viewerEnrollment.status).replace(/_/g, ' ')}
                      </Text>
                    ) : null}
                    {viewerBooking?.status ? (
                      <Text style={{ color: palette.subtext, marginTop: 4 }}>
                        Booking:{' '}
                        {String(viewerBooking.status).replace(/_/g, ' ')}
                      </Text>
                    ) : null}
                    ...
                    {viewerBooking?.provider_completed_at &&
                    !viewerBooking?.payer_satisfied_at ? (
                      <Text
                        style={{
                          color: palette.subtext,
                          marginTop: 8,
            ...
            {workspaceSection === 'overview' ? (
              <DetailSummaryCard palette={palette} summary={detailSummary} />
            ) : null}
            ...
            >
              ...
              >
                {breadcrumbParts.map((part, index) => (
                  ...
                  >
                    ...
                    {index < breadcrumbParts.length - 1 ? (
                      <Text style={{ color: palette.subtext, marginLeft: 8 }}>
                        ›
                      </Text>
                    ) : null}
                  </View>
              ...
              >
                Start with Overview, move through Path, continue in the active
                lesson area, and collect or share your verified certificate when
                you finish.
              </Text>
            </View>
                  ...
                  {outline.map((module, index) => {
                    ...
                    return (
                      <Pressable
                        ...
                        accessibilityRole="button"
                        accessibilityState={{ selected: active }}
                        accessibilityLabel={`Open module ${index + 1}: ${
                          module.title
                        }`}
            ...
            {workspaceSection === 'path' && activeModule ? (
              ...
              >
                ...
                >
                  {activeModule.item_count ?? activeModuleItems.length} items ·{' '}
                  {activeModule.duration_minutes ?? 0} minutes
                </Text>
                  ...
                  {activeModuleItems.map(learningItem => {
                    ...
                    return (
                      <Pressable
                        ...
                        accessibilityLabel={`Open ${formatTypeLabel(
                          learningItem.type,
                        )} ${learningItem.title}`}
                        onPress={() => {
                          if (!hasLearningAccess) {
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            {formatTypeLabel(learningItem.type)}
                          </Text>
                          <Text
                            style={{
                              color: isDone
                          ...
                          <View style={{ flexDirection: 'row', gap: 8 }}>
                            ...
                            {enrolled ? (
                              <KISButton
                                ...
                                disabled={isDone || progressBusy}
                                onPress={() => handleCompleteItem(learningItem)}
                              />
                            ) : null}
                          </View>
                        </View>
            ...
            hasLearningAccess ? (
              ...
              >
                ...
                <Text style={{ color: palette.subtext, marginTop: 4 }}>
                  ...
                  {selectedItem.duration_minutes ?? 0} mins
                </Text>
                ...
                >
                  <KISButton
                    ...
                    size="sm"
                    variant="outline"
                    onPress={() => handleOpenItem(selectedItem)}
                  />
                  {enrolled && !completedItemIds.has(selectedItem.id) ? (
                    <KISButton
                      title="Complete item"
                      size="sm"
                      onPress={() => handleCompleteItem(selectedItem)}
                      disabled={progressBusy}
                    />
                  ) : null}
                </View>
                {nextItem ? (
                  ...
                  >
                    ...
                    >
                      {formatTypeLabel(nextItem.type)} ·{' '}
                      {nextItem.duration_minutes ?? 0} mins
                    </Text>
                    <KISButton
                      title="Go to next item"
                      size="xs"
                      variant="outline"
                      onPress={() => handleOpenItem(nextItem)}
                      style={{ marginTop: 10 }}
                    />
                  </View>
                ) : null}
              ...
              >
                    ...
                    >
                      ...
                      >
                        {certificateLoading
                          ...
                          : 'Open this tab again in a moment if the preview is still loading.'}
                      </Text>
                    </View>
                  )}
                </View>
                ...
                >
                  ...
                  <KISButton
                    ...
                    size="sm"
                    variant="outline"
                    onPress={() => void handleShareCertificate()}
                  />
                </View>
              </View>
            ) : null}
            ...
            {workspaceSection === 'overview' &&
            (content as any)?.institutionSummary ? (
              <View
                style={{
                  marginTop: 18,
                  borderRadius: 22,
              ...
              >
                ...
                >
                  {(content as any).institutionSummary.name}
                </Text>
                ...
                >
                  <Text style={{ color: palette.subtext, fontSize: 12 }}>
                    {(content as any).institutionSummary.courseCount ?? 0}{' '}
                    courses
                  </Text>
                  <Text style={{ color: palette.subtext, fontSize: 12 }}>
                    {(content as any).institutionSummary.programCount ?? 0}{' '}
                    programs
                  </Text>
                  <Text style={{ color: palette.subtext, fontSize: 12 }}>
                    {(content as any).institutionSummary.memberCount ?? 0}{' '}
                    members
                  </Text>
                </View>
              </View>
                  ...
                  {[
                    {
                      label: 'Modules',
                      value: String(
                        (content as any).trustSignals.moduleCount ?? 0,
                      ),
                    },
                    {
                      label: 'Learning items',
                      value: String(
                        (content as any).trustSignals.itemCount ?? 0,
                      ),
                    },
                    {
                      label: 'Assessments',
                      value: String(
                        (content as any).trustSignals.assessmentCount ?? 0,
                      ),
                    },
                    {
                      label: 'Live sessions',
                      value: String(
                        (content as any).trustSignals.liveSessionCount ?? 0,
                      ),
                    },
                    {
                      label: 'Materials',
                      value: String(
                        (content as any).trustSignals.materialCount ?? 0,
                      ),
                    },
                    {
                      label: 'Learners',
                      value: String(
                        (content as any).trustSignals.enrollmentCount ?? 0,
                      ),
                    },
                  ].map(card => (
                    <View
            ...
            {workspaceSection === 'overview' &&
            Array.isArray((content as any)?.instructors) &&
            (content as any).instructors.length ? (
              <View
                style={{
                  marginTop: 18,
                  borderRadius: 22,
          ...
          >
            ...
            <View style={{ flexDirection: 'row', gap: 8 }}>
              ...
              <KISButton
                ...
                onPress={() =>
                  enrolled
                    ? handleOpenItem(selectedItem || flatItems[0])
                    : onEnroll(content)
                }
                disabled={
                  progressBusy ||
                  (enrolled && !selectedItem && flatItems.length === 0)
                }
              />
            </View>
          </View>
