...
class RNAudioRecorderPlayerModule(
    ...
    fun startRecorder(path: String, audioSet: ReadableMap?, meteringEnabled: Boolean, promise: Promise) {
        try {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                ...
                if (activity == null) {
                    Log.w(tag, "Current activity is null while requesting permissions.")
                    promise.reject("No permission granted.", "Try again when activity is available.")
                    return
                }
        ...
        val outputFormat =
            if (audioSet != null && audioSet.hasKey("OutputFormatAndroid"))
                audioSet.getInt("OutputFormatAndroid")
            else
                MediaRecorder.OutputFormat.MPEG_4

        audioFileURL =
            if ((path == "DEFAULT")) "${reactContext.cacheDir}/sound.${defaultFileExtensions[outputFormat]}"
            else path
        ...
        try {
            if (audioSet == null) {
                ...
                newMediaRecorder.setAudioSource(
                ...
                newMediaRecorder.setOutputFormat(outputFormat)
                newMediaRecorder.setAudioEncoder(
                    if (audioSet.hasKey("AudioEncoderAndroid"))
                        audioSet.getInt("AudioEncoderAndroid")
                    else
                        MediaRecorder.AudioEncoder.AAC
                )

                if (audioSet.hasKey("AudioSamplingRateAndroid")) {
                    newMediaRecorder.setAudioSamplingRate(
                        audioSet.getInt("AudioSamplingRateAndroid")
                    )
                }

                if (audioSet.hasKey("AudioEncodingBitRateAndroid")) {
                    newMediaRecorder.setAudioEncodingBitRate(
                        audioSet.getInt("AudioEncodingBitRateAndroid")
                    )
                }

                if (audioSet.hasKey("AudioChannelsAndroid")) {
                    newMediaRecorder.setAudioChannels(
